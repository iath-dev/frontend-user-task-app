...
describe('Table component', () => {
  ...
  it('renders table header with correct columns', () => {
    render(
        <Table data={mockUsers} columns={columns} rowComponent={(item) => (
            <tr key={item.id}>
                <td>{item.name}</td>
                <td>{item.email}</td>
                <td>{item.id}</td>
  ...
  it('renders table body with correct rows', () => {
    render(
        <Table data={mockUsers} columns={columns} itemsPerPage={7} rowComponent={(item) => (
            <tr key={item.id}>
                <td>{item.name}</td>
                <td>{item.email}</td>
                <td>{item.id}</td>
  ...
  it('renders table body with correct rows, limit data', () => {
    ...
    render(
        <Table data={sliceUsers} columns={columns} itemsPerPage={5} rowComponent={(item) => (
            <tr key={item.id}>
                <td>{item.name}</td>
                <td>{item.email}</td>
                <td>{item.id}</td>
